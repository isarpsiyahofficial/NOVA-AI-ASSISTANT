...
class _CallContactControlPageState extends State<CallContactControlPage> {
  ...
  Future<void> _changeContactRole(NovaContact contact, ContactRole role) async {
    ...
    final normalizedCustomRoleLabel = customRoleLabel?.trim();
  ...
  Widget _instructionModeDropdown({
    required String label,
    required ContactInstructionMode value,
    required ValueChanged<ContactInstructionMode> onChanged,
  }) {
    return DropdownButtonFormField<ContactInstructionMode>(
      initialValue: value,
      items: ContactInstructionMode.values
          .map(
            (mode) => DropdownMenuItem<ContactInstructionMode>(
              value: mode,
  ...
  Future<void> _editContactCustomization(NovaContact contact) async {
    ...
    final customRoleController = TextEditingController(
      text: contact.customRoleLabel ?? '',
    );
    var selectedRole = contact.role;
    var openingMode = contact.callOpeningInstructionMode;
    var questionsMode = contact.expectedCallerQuestionsMode;
    var allowedMode = contact.allowedResponseScopeMode;
    var blockedMode = contact.blockedResponseScopeMode;
    var emergencyMode = contact.emergencyInstructionMode;
    var whiteLieMode = contact.whiteLieInstructionMode;
    var quickReplyMode = contact.quickReplyInstructionMode;
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (selectedRole == ContactRole.custom) ...[
                        const SizedBox(height: 12),
                        TextField(
                          controller: customRoleController,
                          decoration: const InputDecoration(
                            labelText: 'Özel rol / ilişki etiketi',
                            border: OutlineInputBorder(),
                          ),
                        ),
                      ],
                      const SizedBox(height: 12),
                      _instructionModeDropdown(
                        label: 'Özel açılış / karşılama',
                        value: openingMode,
                        onChanged: (mode) =>
                            setModalState(() => openingMode = mode),
                      ),
                      const SizedBox(height: 12),
                      TextField(
                      ...
                      const SizedBox(height: 12),
                      _instructionModeDropdown(
                        label: 'Bu kişi ne sorabilir',
                        value: questionsMode,
                        onChanged: (mode) =>
                            setModalState(() => questionsMode = mode),
                      ),
                      const SizedBox(height: 12),
                      TextField(
                      ...
                      const SizedBox(height: 12),
                      _instructionModeDropdown(
                        label: 'Cevap verebileceği kapsam',
                        value: allowedMode,
                        onChanged: (mode) =>
                            setModalState(() => allowedMode = mode),
                      ),
                      const SizedBox(height: 12),
                      TextField(
                      ...
                      const SizedBox(height: 12),
                      _instructionModeDropdown(
                        label: 'Girmeyeceği konular',
                        value: blockedMode,
                        onChanged: (mode) =>
                            setModalState(() => blockedMode = mode),
                      ),
                      const SizedBox(height: 12),
                      TextField(
                      ...
                      const SizedBox(height: 12),
                      _instructionModeDropdown(
                        label: 'Acil durum davranışı',
                        value: emergencyMode,
                        onChanged: (mode) =>
                            setModalState(() => emergencyMode = mode),
                      ),
                      const SizedBox(height: 12),
                      TextField(
                      ...
                      const SizedBox(height: 12),
                      _instructionModeDropdown(
                        label: 'Sosyal uygun cevap çizgisi',
                        value: whiteLieMode,
                        onChanged: (mode) =>
                            setModalState(() => whiteLieMode = mode),
                      ),
                      const SizedBox(height: 12),
                      TextField(
                      ...
                      const SizedBox(height: 12),
                      _instructionModeDropdown(
                        label: 'Hazır kısa yanıt',
                        value: quickReplyMode,
                        onChanged: (mode) =>
                            setModalState(() => quickReplyMode = mode),
                      ),
                      const SizedBox(height: 12),
                      TextField(
                ...
                FilledButton(
                  onPressed: () {
                    Navigator.of(context).pop(
                      contact.copyWith(
                        role: selectedRole,
                        customRoleLabel: selectedRole == ContactRole.custom
                            ? customRoleController.text.trim()
                            : null,
                        relationshipLabel: selectedRole == ContactRole.custom
                            ? customRoleController.text.trim()
                            : selectedRole.label,
                        ...
                        callOpeningInstructionMode: openingMode,
                        expectedCallerQuestionsMode: questionsMode,
                        allowedResponseScopeMode: allowedMode,
                        blockedResponseScopeMode: blockedMode,
                        emergencyInstructionMode: emergencyMode,
                        whiteLieInstructionMode: whiteLieMode,
                        quickReplyInstructionMode: quickReplyMode,
