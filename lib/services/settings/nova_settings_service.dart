...
// NOVA_API_FIRST_SECURE_SETTINGS_V2_CARRIER_BRIDGE_SECRET
import 'dart:convert';
...
class NovaSettingsService {
  ...
  Future<NovaSettings> load() async {
    try {
      ...
      final tokenStore = const NovaSecureTokenStore();
      final secureKey = await tokenStore.read(base.activeAiProvider);
      final bridgeToken = await tokenStore.readNamed(
        NovaSecureTokenStore.carrierBridgeTokenName,
      );
      return base.copyWith(
        apiKey: secureKey.trim().isNotEmpty ? secureKey : base.apiKey,
        carrierBridgeControlToken: bridgeToken.trim().isNotEmpty
            ? bridgeToken
            : base.carrierBridgeControlToken,
      );
    } catch (_) {
  ...
  Future<void> save(NovaSettings settings) async {
    ...
    await tokenStore.writeNamed(
      NovaSecureTokenStore.carrierBridgeTokenName,
      settings.carrierBridgeControlToken,
    );
    final prefs = await SharedPreferences.getInstance();
