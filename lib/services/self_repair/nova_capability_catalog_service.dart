...
class NovaCapabilityCatalogService {
  ...
  Future<List<NovaCapabilityCatalogItem>> loadSafeCatalog() async {
    final manifest = await recognitionService.manifestService.loadManifest();
    await recognitionService.manifestService.runtimeRegistryService
        ?.registerCapabilities(manifest);
    final descriptors = await recognitionService.discoverCapabilities();
    final probes = await probeService.probeAll(manifest);
