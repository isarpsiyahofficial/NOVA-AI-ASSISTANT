...
class NovaSelfRepairCommandService {
  ...
  NovaSelfRepairCommandParseResult parse(String input) {
    ...
    final mappedArea = requestedArea.trim().isEmpty
        ? _mapKeywordArea(lower)
        : requestedArea.trim();
    return NovaSelfRepairCommandParseResult(
      matched: true,
      wantsRepair: true,
      requestedArea: mappedArea,
      message: mappedArea.isEmpty
          ? 'Genel onarım isteği algılandı.'
          : '$mappedArea alanı için onarım isteği algılandı.',
    );
  }
  ...
  String _mapKeywordArea(String lower) {
    final text = lower.trim();
    if (text.contains('asr') ||
        text.contains('stt') ||
        text.contains('mikrofon') ||
        text.contains('microphone') ||
        text.contains('dinleme') ||
        text.contains('duymuyor') ||
        text.contains('beni duymuyor') ||
        text.contains('sesimi algılamıyor') ||
        text.contains('sesimi algilamiyor')) {
      return 'listening_runtime';
    }
    if (text.contains('robotik') ||
        text.contains('robot gibi') ||
        text.contains('fallback') ||
        text.contains('sabit metin') ||
        text.contains('konuşma') ||
        text.contains('konusma') ||
        text.contains('tts') ||
        text.contains('cevap vermiyor')) {
      return 'speech_response';
    }
    if (text.contains('overlay') ||
        text.contains('ekran üstü') ||
        text.contains('ekran ustu') ||
        text.contains('baloncuk') ||
        text.contains('katman görünmüyor') ||
        text.contains('katman gorunmuyor')) {
      return 'overlay_background';
    }
    if (text.contains('gemma') ||
        text.contains('yerel model') ||
        text.contains('local model') ||
        text.contains('native inference') ||
        text.contains('model çıkarımı') ||
        text.contains('model cikarimi')) {
      return 'ai_response';
    }
    if (text.contains('setup') ||
        text.contains('kurulum') ||
        text.contains('adım geçiyor') ||
        text.contains('adim geciyor') ||
        text.contains('yanlış isim') ||
        text.contains('yanlis isim')) {
      return 'setup_lifecycle';
    }
    if (text.contains('izin') ||
        text.contains('permission') ||
        text.contains('mikrofon izni') ||
        text.contains('dashboard')) {
      return 'permissions_runtime';
    }
    return '';
  }
}
